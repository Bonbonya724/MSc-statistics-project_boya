...
max.effect <- -Delta.AB + 1
min.effect <- -Delta.AB - 1
...
for (i in 1:scenarios) {
  ...
  load(paste0("Results/Bucher/variances_", file.id, ".RData")) #file.id /file.id
...
# par(pty="m")
# plot(pd.cover$maic.vr, type="n", ylim=c(0.8,3), bty="n", xlab="Scenario", ylab="Variability ratio",
#      las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
# abline(h=1, col="grey") # unbiased variance
# lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
# lines(pd.cover, which="r", ymin.refline=2, ymax.refline=3, cex.ref=0.9)
# lines(pd.cover$maic.vr, col="red", lty=2,type="s",lwd=2.5)
# lines(pd.cover$stc.vr, col="green", lty=5, type="s", lwd=2.5)
# lines(pd.cover$bucher.vr, col="blue", lty=4, type="s", lwd=2.5)
# # lines(pd.cover$bucher.vr, col="orange", lty=4, type="s", lwd=2.5)
...
pdf("Analysis/variability_ratio.pdf", width=18,height=15,pointsize=20)
par(pty="m")
plot(pd.cover$maic.vr, type="n", ylim=c(0.5,2), bty="n", xlab="Scenario", ylab="Variability ratio",
     las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
abline(h=1, col="grey") # unbiased variance
lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
...
lines(pd.mse, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.mse, which="r",ymin.refline=0.7, ymax.refline=1,cex.ref=0.9)
...
par(pty="m")
plot(pd.bias$maic.std.bias, ylim=c(-600,400), bty="n", xlab="Scenario", ylab="Standardized percentage bias",
     las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
abline(h=0, col="grey") # no bias
lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.bias, which="r", ymin.refline=200, ymax.refline=400, cex.ref=0.9)
...
par(pty="m")
plot(pd.cover$maic.error.worse.than*100, type="n", ylim=c(0, 120), bty="n", xlab="Scenario",
     ylab="% of point estimates worse than Bucher", las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.cover, which="r", ymin.refline=100, ymax.refline=120, cex.ref=0.9)
