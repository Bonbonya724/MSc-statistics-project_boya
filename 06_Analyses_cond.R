...
## Conditional graphs ylim different specification 
...
load(file="survival_settings_condff.RData") 
#load(file="survival_settings_margff.RData")

## add this line if runs v2
#replicates<- 2000

source("02_Functions.R") # load functions to compute performance measures and for plotting
# use Nelder-Mead method in optim()
# source("functions.R") # original code use BFGS method in optim()

# TRUE VALUE OF TARGET ESTIMAND IS ZERO
#-------------------------------------------------------------------------------
#ADJUSTED THE DELTA.AB from ZERO to the below
Delta.AB <- log(0.25)-ifelse(effects=="marg", log(0.25),log(0.4))
# marginal setting: Delta.AB=0
# conditional Delta.AB= log(0.25)-log(0.4)=log(0.625)
...
max.effect <- -Delta.AB +0.5
min.effect <- -Delta.AB -0.5
...
for (i in 1:scenarios) {
  ...
  # 
  ## change file.id when running v1
  file.id <- paste0("N_AC", pc$N_AC[i], "b_X", round(pc$b_X[i], digits=2),
                    "b_EM", round(pc$b_EM[i], digits=2),
                    "meanX_AC", pc$meanX_AC[i], "corX", pc$corX[i])

  ## change file.id when running v2
  # file.id2 <- paste0("N_AC", pc$N_AC[i], "b_X", round(pc$b_X[i], digits=2),
  #                   "b_EM", round(pc$b_EM[i], digits=2),
  #                   "meanX_AC", pc$meanX_AC[i], "corX", pc$corX[i])
  ...
  # number of replicates for which MAIC weighted regression has separability issues 
  maic.separability.list[i] <- sum(abs(means[!is.na(means)])>max.effect | abs(means[!is.na(means)])<min.effect) 
  ...
  # maic.no.wei.pro <- !is.na(means) & !is.na(variances) & variances<5
  maic.no.wei.pro <- !is.na(means) & !is.na(variances)
  #means[!maic.no.wei.pro] <- mean(means[abs(means)<max.effect & !is.na(means)])
  #variances[!maic.no.wei.pro] <- mean(variances[variances<100])
  means <- means[maic.no.wei.pro]
  variances <- variances[maic.no.wei.pro]
  maic.no.sep <- abs(means)<max.effect & abs(means)>min.effect
  #maic.no.sep <- abs(means)<max.effect
  #m1[i] <- mean(means[abs(means)<max.effect & !is.na(means)])
  #means[!maic.no.sep] <- rnorm(sum(abs(means)>max.effect),m1[i],abs(m1[i]))
  #variances[variances>0.1] <- mean(variances[variances<0.1])
  ...
  load(paste0("Results/STC/means_", file.id, ".RData")) #file.id / file.id2
  load(paste0("Results/STC/variances_", file.id, ".RData")) #file.id / file.id2
  ...
  load(paste0("Results/Bucher/means_", file.id, ".RData")) #file.id / file.id2
  load(paste0("Results/Bucher/variances_", file.id, ".RData")) #file.id / file.id2
  ...
  # load(paste0("Results/MLNMR/means_", file.id, ".RData"))
  # load(paste0("Results/MLNMR/variances_", file.id, ".RData"))  
  ...
  # tmp.scenarios<- cbind(tmp.scenarios, maic.tmp.metrics, stc.tmp.metrics, bucher.tmp.metrics, mlnmr.tmp.metrics)
  ...
  # bias_all <-cbind(bucher_bias_nonsum,maic_bias_nonsum,stc_bias_nonsum,mlnmr_bias_nonsum) 
  bias_all <-cbind(bucher_bias_nonsum,maic_bias_nonsum,stc_bias_nonsum) 
}  
...
write.csv(scenarios.df, paste0("Analysis/scenarios/",effects,".csv"), row.names = FALSE)

scenarios.df <- read.csv(file=paste0("Analysis/scenarios/",effects,".csv"), header=TRUE, sep=",")
...
g <- ggplot(longbias_all, aes(`ITC method`, Bias)) +
  geom_point(shape=1)
pdf("Analysis/ITC_bias.pdf",  width = 7, height=5)
par(pty="m")
print(g)
dev.off()
...
# (function is by R?cker, G., Schwarzer, G. Presenting simulation results in a nested loop plot. 
...
# # lines(pd.mse$mlnmr.mse, col="orange", lty=4, type="s", lwd=2.5)
...
lines(pd.mse, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.mse, which="r", ymin.refline=0.8, ymax.refline=1, cex.ref=0.9) 
lines(pd.mse$maic.mse, col="red", lty=2,type="s", lwd=2.5)
lines(pd.mse$stc.mse, col="green", lty=5, type="s", lwd=2.5)
lines(pd.mse$bucher.mse, col="blue", lty=4, type="s", lwd=2.5)
# lines(pd.mse$bucher.mse, col="orange", lty=4, type="s", lwd=2.5)
legend("topright",lwd=c(2.5,2.5,2.5),col=c("red","green","blue"), lty=c(2,5,4),
...
# # lines(pd.ese$bucher.empse, col="orange", lty=4, type="s", lwd=2.5) 
# legend("topright", lwd=c(2.5,2.5,2.5), col=c("red","green","blue"), lty=c(2,5,4), 
#        cex=1.5, bty="n", c("MAIC", "STC", "Bucher"))
# dev.off()
...
par(pty="m")
plot(pd.ese$maic.empse, type="n",ylim=c(0, 1), bty="n", xlab="Scenario",
     ylab="Empirical standard error (ESE)",las=1, xaxt="n",
     cex.axis=1.15, cex.lab=1.15)
lines(pd.ese, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.ese, which="r", ymin.refline=0.8, ymax.refline=1, cex.ref=0.9)
lines(pd.ese$maic.empse, col="red", lty=2,type="s",lwd=2.5)    
lines(pd.ese$stc.empse, col="green", lty=5, type="s", lwd=2.5)   
lines(pd.ese$bucher.empse, col="blue", lty=4, type="s", lwd=2.5) 
# lines(pd.ese$bucher.empse, col="orange", lty=4, type="s", lwd=2.5) 
...
# # lines(pd.cover$bucher.cov*100, col="orange", lty=4, type="s", lwd=2.5) 
...
abline(h=95, col="grey") # nominal 
lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.cover, which="r", ymin.refline=110, ymax.refline=140, cex.ref=0.9)
lines(pd.cover$maic.cov*100, col="red", lty=2,type="s", lwd=2.5)    
lines(pd.cover$stc.cov*100, col="green", lty=5, type="s", lwd=2.5) 
lines(pd.cover$bucher.cov*100, col="blue", lty=4, type="s", lwd=2.5)
# lines(pd.cover$bucher.cov*100, col="orange", lty=4, type="s", lwd=2.5) 
legend("topright", lwd=c(2.5,2.5,2.5), col=c("red","green", "blue"), 
...
# # lines(pd.cover$bucher.vr, col="orange", lty=4, type="s", lwd=2.5) 
# legend("topright", lwd=c(2.5,2.5,2.5), col=c("red","green","blue"),lty=c(2,5,4), 
#        cex=1.5, bty="n", c("MAIC", "STC", "Bucher"))
# dev.off()
...
# par(pty="m")
# plot(pd.cover$maic.vr, type="n", ylim=c(0,2), bty="n", xlab="Scenario", ylab="Variability ratio",
#      las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
# abline(h=1, col="grey") # unbiased variance
# lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
# lines(pd.cover, which="r", ymin.refline=1.5, ymax.refline=2, cex.ref=0.9)
# lines(pd.cover$maic.vr, col="red", lty=2,type="s",lwd=2.5)    
# lines(pd.cover$stc.vr, col="green", lty=5, type="s", lwd=2.5)   
# lines(pd.cover$bucher.vr, col="blue", lty=4, type="s", lwd=2.5) 
# # lines(pd.cover$bucher.vr, col="orange", lty=4, type="s", lwd=2.5) 
...
pdf("Analysis/variability_ratio.pdf", width=18,height=15,pointsize=20)
par(pty="m")
plot(pd.cover$maic.vr, type="n", ylim=c(0,1.6), bty="n", xlab="Scenario", ylab="Variability ratio",
     las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
abline(h=1, col="grey") # unbiased variance
lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.cover, which="r", ymin.refline=1.3, ymax.refline=1.6, cex.ref=0.9)
lines(pd.cover$maic.vr, col="red", lty=2,type="s",lwd=2.5)    
lines(pd.cover$stc.vr, col="green", lty=5, type="s", lwd=2.5)   
lines(pd.cover$bucher.vr, col="blue", lty=4, type="s", lwd=2.5) 
# lines(pd.cover$bucher.vr, col="orange", lty=4, type="s", lwd=2.5) 
...
# # lines(pd.bias$bucher.bias, col="orange", lty=4, type="s", lwd=2.5)
# legend("topright",lwd=c(2.5,2.5,2.5),col=c("red","green", "blue"),
...
par(pty="m")
plot(pd.bias$maic.bias, type="n", ylim=c(-0.5,1), bty="n", xlab="Scenario", ylab="Bias",
     las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
abline(h=0, col="grey") # no bias
lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE")) # add vertical lines
lines(pd.bias, which="r",ymin.refline=0.6, ymax.refline=1, cex.ref=0.9) # add reference lines
lines(pd.bias$maic.bias, col="red", lty=2,type="s", lwd=2.5) # performance measures
lines(pd.bias$stc.bias, col="green", lty=5, type="s", lwd=2.5)
lines(pd.bias$bucher.bias, col="blue", lty=4, type="s", lwd=2.5)
# lines(pd.bias$bucher.bias, col="orange", lty=4, type="s", lwd=2.5)
...
# pdf("Analysis/bias.pdf", width=18, height=15, pointsize=20)
# par(pty="m")
# plot(pd.bias$maic.bias, type="n", ylim=c(-1,1), bty="n", xlab="Scenario", ylab="Bias",
#      las=1, xaxt="n", cex.axis=1.15, cex.lab=1.15)
# abline(h=0, col="grey") # no bias
# lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE")) # add vertical lines
# lines(pd.bias, which="r",ymin.refline=0.6, ymax.refline=1, cex.ref=0.9) # add reference lines
...
# dev.off()
# 
# pd.bias$maic.bias.adj = pd.bias$maic.bias - c(rep(0,54), rep(log(0.5)-log(0.33),54),rep(log(0.67)-log(0.33),54))
# pd.bias$stc.bias.adj = pd.bias$stc.bias - c(rep(0,54), rep(log(0.5)-log(0.33),54),rep(log(0.67)-log(0.33),54))
# pd.bias$bucher.bias.adj = pd.bias$bucher.bias - c(rep(0,54), rep(log(0.5)-log(0.33),54),rep(log(0.67)-log(0.33),54))
# 
# pdf("Analysis/bias.pdf", width=18, height=15, pointsize=20)
# par(pty="m")
# plot(pd.bias$maic.bias, type="n", ylim=c(-1,1), bty="n", xlab="Scenario", ylab="Bias",
...
# lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE")) # add vertical lines
# lines(pd.bias, which="r",ymin.refline=0.6, ymax.refline=1, cex.ref=0.9) # add reference lines
# lines(pd.bias$maic.bias.adj, col="red", lty=2,type="s", lwd=2.5) # performance measures
# lines(pd.bias$stc.bias.adj, col="green", lty=5, type="s", lwd=2.5)
# lines(pd.bias$bucher.bias.adj, col="blue", lty=4, type="s", lwd=2.5)
# # lines(pd.bias$bucher.bias, col="orange", lty=4, type="s", lwd=2.5)
# legend("topright",lwd=c(2.5,2.5,2.5),col=c("red","green", "blue"),
#        lty=c(2,5,4),cex=1.15,bty="n", c("MAIC", "STC", "Bucher")) # legend
# dev.off()
...
par(pty="m")
plot(pd.mse$maic.mae,type="n",ylim=c(0, 1),bty="n",xlab="Scenario",
...
# lines(pd.mse$bucher.mae, col="orange", lty=4, type="s", lwd=2.5)
legend("topright",lwd=c(2.5,2.5,2.5),col=c("red","green","blue"),lty=c(2,5,4),
       cex=1.15,bty="n",c("MAIC", "STC", "Bucher"))
dev.off()
...
# par(pty="m")
# plot(pd.bias$maic.std.bias, ylim=c(-400,600), bty="n", xlab="Scenario", ylab="Standardized percentage bias",
#      las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
# abline(h=0, col="grey") # no bias
# lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
# lines(pd.bias, which="r", ymin.refline=400, ymax.refline=600, cex.ref=0.9)
...
# # lines(pd.bias$bucher.std.bias, col="orange", lty=4, type="s", lwd=2.5)
# legend("topright",lwd=c(2.5,2.5,2.5), col=c("red","green","blue"), lty=c(2,5,4),
#        cex=1.15, bty="n", c("MAIC", "STC", "Bucher"))
# dev.off()
...
pdf("Analysis/Supplementary_Material/std_biases.pdf", width=18,height=15,pointsize=20)
par(pty="m")
plot(pd.bias$maic.std.bias, ylim=c(-600,600), bty="n", xlab="Scenario", ylab="Standardized percentage bias",
     las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
abline(h=0, col="grey") # no bias
lines(pd.bias, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.bias, which="r", ymin.refline=400, ymax.refline=600, cex.ref=0.9)
...
par(pty="m")
plot(pd.mse$maic.uci.mean-pd.mse$maic.lci.mean, type="n", ylim=c(0, 4), bty="n", xlab="Scenario",
     ylab="95% confidence interval width", las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
lines(pd.mse, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.mse, which="r", ymin.refline=3, ymax.refline=4, cex.ref=0.9)
...
# lines(pd.mse$bucher.uci.mean-pd.mse$bucher.lci.mean, col="orange", lty=4, type="s", lwd=2.5)
legend("topright", lwd=c(2.5,2.5,2.5), col=c("red","green","blue"), lty=c(2,5,4),
       cex=1.15, bty="n", c("MAIC", "STC", "Bucher"))
dev.off()
...
par(pty="m")
plot(pd.cover$maic.error.worse.than*100, type="n", ylim=c(0, 160), bty="n", xlab="Scenario",
     ylab="% of point estimates worse than Bucher", las=1, xaxt="n",cex.axis=1.15, cex.lab=1.15)
lines(pd.cover, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.cover, which="r", ymin.refline=120, ymax.refline=160, cex.ref=0.9)
...
# lines(pd.cover$stc.error.worse.than*100, col="orange", lty=5, type="s", lwd=2.5)
legend("topright", lwd=c(2.5, 2.5), col=c("red","green","blue"), lty=c(2,5),
...
par(pty="m")
plot((pd.aess$N_AC-pd.aess$maic.aess.mean)*100/pd.aess$N_AC, type="n", ylim=c(20,100),
     bty="n", xlab="Scenario", ylab="% reduction in effective sample size", las=1, xaxt="n",
     cex.axis=1.15, cex.lab=1.15)
lines(pd.aess, col=c("#BBBBBB", "#CCCCCC", "#DDDDDD", "#EEEEEE"))
lines(pd.aess, which="r", ymin.refline=80, ymax.refline=100, cex.ref=0.9)
lines((pd.aess$N_AC-pd.aess$maic.aess.mean)*100/pd.aess$N_AC, col="red", lty=2,type="s", lwd=2.5)
legend("topright", lwd=c(2.5), col=c("red"), lty=c(2), cex=1.15, bty="n", c("MAIC"))
dev.off()
...
plot(maic.rolling.bias.1, xlab="Simulation number", ylab="Rolling bias", col="red",
     type="l",ylim=c(-1,0), lwd=2, lty=2, cex.axis=1.15, cex.lab=1.15)
abline(h=0, col="grey") # no bias
